...
function PortfolioPage({ isPaper }: { isPaper: boolean }) {
  ...
  // Settled wallet values stay server-authoritative. Open P&L is marked to the
  // latest quote locally until the live position stream is wired in.
  ...
  const portfolio = { ...basePortfolio, positions: dynamicPositions, totalPnl: dynamicTotalPnl };
  const walletBalance = basePortfolio.walletBalance;
  const availableBalance = basePortfolio.availableBalance;
  const marginUsed = Math.max(0, walletBalance - availableBalance);
  const closeOne = (id: string) => {
    setNotice('');
    setNoticeIsError(false);
    closePosition.mutate({ id }, {
      onSuccess: () => { setNotice('Position closed — settled balance and margin are updated.'); portfolioQuery.refetch(); },
      onError: () => { setNotice('Position could not be closed. No balance was changed.'); setNoticeIsError(true); },
    });
  };
  const closeEverything = () => {
    ...
    closeAll.mutate(undefined, {
      onSuccess: () => { setNotice('All paper positions closed and settled.'); portfolioQuery.refetch(); },
      onError: () => { setNotice('Close all failed. Check each open position before trying again.'); setNoticeIsError(true); },
    });
  };
  const addFunds = () => {
    setNotice('');
    setNoticeIsError(false);
    addFundsMutation.mutate({ data: { amount: 50000 } }, {
      onSuccess: () => { setNotice('₹50,000 mock funds added to settled balance.'); portfolioQuery.refetch(); },
      onError: () => { setNotice('Could not add funds — please try again.'); setNoticeIsError(true); },
    });
  };
  ...
  return <div className="space-y-7">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>
        <div className="flex flex-wrap items-center gap-2"><Badge tone={isPaper ? 'amber' : 'red'}>{isPaper ? 'Paper book' : 'Live book'}</Badge><span className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-emerald-700"><span className="h-1.5 w-1.5 animate-pulse-dot rounded-full bg-emerald-500" /> Marking live</span></div>
        <h2 className="mt-3 text-2xl font-extrabold tracking-[-.04em] md:text-3xl">Your trading book.</h2>
        <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Know what is settled, what is at risk, and what can move next.</p>
      </div>
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" disabled={!isPaper || addFundsMutation.isPending} onClick={addFunds} className="h-9 px-3" data-testid="button-add-funds">{addFundsMutation.isPending ? 'Adding…' : 'Add ₹50,000'}<Plus size={14} /></Button>
        <Button variant="danger" disabled={!isPaper || portfolio.positions.length === 0 || closeAll.isPending} onClick={closeEverything} className="h-9 px-3" data-testid="button-close-all">{closeAll.isPending ? 'Closing…' : 'Close all'}<X size={14} /></Button>
      </div>
    </div>
    {portfolioQuery.isError && <div className="flex items-center justify-between gap-3 rounded-md border border-red-200 bg-red-50 px-3 py-2.5 text-xs font-semibold text-red-700" data-testid="status-portfolio-error"><span>Portfolio data could not be refreshed. Showing the last available book.</span><Button variant="quiet" onClick={() => portfolioQuery.refetch()} className="h-7 px-2 text-red-700">Retry</Button></div>}
    ...
    <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
      {[
        ['Settled balance', money(walletBalance), isPaper ? 'Paper wallet · INR' : 'Broker balance · INR', IndianRupee],
        ['Available to trade', money(availableBalance), 'After open-position margin', IndianRupee],
        ['Open P&L · gross', `${openPnlPositive ? '+' : ''}${money(portfolio.totalPnl)}`, 'Live mark · before fees', openPnlPositive ? TrendingUp : ArrowDownRight],
        ['Margin used', money(marginUsed), `${exposurePercent.toFixed(0)}% of settled balance`, LockKeyhole],
      ].map(([label, value, sub, Icon], index) => <div key={label as string} className={cx('rounded-lg border bg-[hsl(var(--card))] p-4 shadow-[var(--shadow-xs)]', index === 2 && (openPnlPositive ? 'border-emerald-200' : 'border-red-200'))}><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-[hsl(var(--muted-foreground))]">{label as string}</p><Icon size={15} className={cx(index === 2 ? (openPnlPositive ? 'text-emerald-600' : 'text-red-600') : 'text-[hsl(var(--muted-foreground))]')} /></div><p className={cx('mt-3 font-mono text-lg font-medium', index === 2 && (openPnlPositive ? 'text-emerald-700' : 'text-red-700'))} data-testid={`text-portfolio-${index}`}>{value as string}</p><p className="mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">{sub as string}</p></div>)}
    </section>
    <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50/70 px-4 py-3 text-[11px] leading-5 text-amber-900"><ShieldCheck size={16} className="mt-0.5 shrink-0 text-amber-700" /><p><span className="font-bold">P&L clarity:</span> open positions show gross, unrealized P&L against the live mark. Once a trade closes, realized P&L settles net of applicable exchange fees. Any post-tax figure is an estimate only — not tax advice.</p></div>
